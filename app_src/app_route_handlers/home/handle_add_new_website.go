...
const kPostArgPasswordEncrypted = "passwordEncrypted"
...
func (hh *HomeHandler) handleAddOrModifyUserWebsiteCredentials(user *identity_service.UserBlob, httpResponseWriter http.ResponseWriter, request *http.Request, args *controller.HandlerFuncArgs) {
	...
	err = userWebsites.AddOrModifyUserWebsiteCredentials(parsedArgs.WebsiteName, parsedArgs.UserAlias, parsedArgs.PasswordEncrypted, request.Context())
	if err != nil {
...
func parseAddNewWebsitePostArgs(postArgs map[string]string) (*AddNewWebsitePostArgs, error) {
	...
	passwordEncrypted, ok := postArgs[kPostArgPasswordEncrypted]
	if !ok || len(passwordEncrypted) == 0 {
		return nil, errors.New("* password-encrypted cannot be empty")
	}
